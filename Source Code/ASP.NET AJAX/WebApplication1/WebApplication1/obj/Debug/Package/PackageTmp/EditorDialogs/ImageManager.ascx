...
Telerik.Web.UI.Widgets.ImagePreviewer.prototype = {
	...
	_initializeChildren: function()
	{
		...
		//compensate for larger images so they don't show scrollbars
		this._containerBounds.height = this._containerBounds.height - 3;
	...
	_fitImage: function()
	{
		...
		var newSize = { width: Math.floor(imageBounds.width / ratio), height: Math.floor(imageBounds.height / ratio) };
		this._adjustImage(newSize);
	},
	...
	_isImageAThumb: function(item)
	{
		this._thumbOriginalUrl = null;
		if (item.get_type() == Telerik.Web.UI.FileExplorerItemType.Directory) return false;

		//check if image ends with suffix
		var itemName = item.get_name();
		var suffix = this.get_imageEditorFileSuffix();
		if (!suffix) return false;
		suffix += ".";
		if (itemName && itemName.indexOf(suffix) == -1) return false;

		//check if original exists
		var fileExplorer = this.get_browser().get_fileBrowser();
		var originalPath = item.get_path();
		try
		{
			var originalExt = originalPath.substring(originalPath.lastIndexOf(suffix)).replace(suffix, ".");
			originalPath = originalPath.substring(0, originalPath.lastIndexOf(suffix)) + originalExt;
			var originalItem = fileExplorer._findItemByPath(originalPath);
			if (originalItem)
			{
				this._thumbOriginalUrl = originalItem.get_url();
				return true;
			}
		}
		catch (exc) { };
	...
	setItem: function(item)
	{
		...
		this._imageProperties.showThumbRow(this._isImageAThumb(item));
	},
	...
	getResult: function()
	{
		...
		{
			if (!newImage.style.border) newImage.style.border = "0px none blue";
